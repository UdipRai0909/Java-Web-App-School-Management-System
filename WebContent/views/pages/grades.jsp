...
<c:if test="${sessionScope.first_name == null || sessionScope.role_id == null}">
...
<t:layout>
 <jsp:attribute name="title">
  ...
  <c:if test="${createParam == null && editParam == null && displayParam == null}">
   <title>Grades</title>
  </c:if>
  <c:if test="${createParam == 'selected' && editParam == null && displayParam == null}">
   <title>Create Grade</title>
  </c:if>
  <c:if test="${createParam == null && editParam == 'selected' && displayParam == null}">
        ...
        <tr>
         <th scope="row">
          ...
          <form method="get" action="ServletGrade" class="">
           <button type="submit" name="btnDisplay" class="btn btn-info" value="${grd.getGrdId()}">
         ...
         <td style="height: 150px;">
          <div class="scrollable custom-scrollbar">${grd.getGrdRmk()}</div>
         </td>

         <c:if test="${sessionScope.role_id == 1 || sessionScope.role_id == 2}">
          <td>
           <form method="get" action="ServletGrade" class="">
            <button name="btnEdit" type="submit" value="${grd.getGrdId()}" class="btn btn-edit">
             <i class="fas fa-lg fa-edit"></i>
            </button>
           </form>

           <form method="post" action="ServletGrade" class="pt-3">
            <button name="postDelete" class="btn btn-delete" value="${grd.getGrdId()}" type="submit">
     ...
     <form method="post" action="ServletGrade">
      ...
      <div class="form-row">
       <div class="form-group col-md-6">
        ...
        <input type="text" class="form-control" id="crGrdStd" name="crGrdStd">
       </div>
       <div class="form-group col-md-6">
        <label for="crGrdPer">Percentage</label>
        <input type="number" class="form-control" step="0.01" id="crGrdPer" name="crGrdPer">
       </div>
      </div>
      ...
      <div class="form-row">
       <div class="form-group col-md-12">
        ...
        <textarea class="form-control" rows="5" id="crGrdRmk" name="crGrdRmk"> </textarea>
       </div>
      </div>

      <button name="postCreate" value="create" type="submit" class="btn btn-create">Create</button>
     </form>
    </div>
     ...
     <form method="post" action="ServletGrade">
      ...
      <div class="form-row">
       <div class="form-group col-md-6">
        ...
        <input type="text" class="form-control" id="upGrdStd" name="upGrdStd" value="<c:out value= "
         ${grade.getGrdStd()}" />">
       </div>
       <div class="form-group col-md-6">
        <label for="upGrdPer">Percentage</label>
        <input type="number" class="form-control" id="upGrdPer" step="0.01" name="upGrdPer" value="<c:out value= "
         ${grade.getGrdPer()}" />">
       </div>
      </div>
      ...
      <button name="postUpdate" value='<c:out value= "${grade.getGrdId()}"/>' type="submit"
     ...
     <table class="table table-striped table-dark w-50 text-left mt-5">
      ...
      <tbody>
       ...
       <tr>
        ...
        <td style="height: 150px;" class="pb-4">
         <div class="scrollable custom-scrollbar">${grade.getGrdRmk()}</div>
        </td>
       </tr>
      </tbody>
     </table>
