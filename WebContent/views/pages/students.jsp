...
<c:if
	test="${sessionScope.role_id == null && registerStudentRole == null}">
...
<t:layout>
	<jsp:attribute name="title">
		...
		<c:if
			test="${createParam == null && editParam == null && displayParam == null && registerStudentRole == null}">
			<title>Students</title>
		</c:if>
		<c:if
			test="${createParam == 'selected' && registerStudentRole != null}">
				 ...
				 <c:if
					...
					<form method="get" action="ServletStudent">
						<button name="btnCreate" type="submit" value="create"
							...
							<h6 class="pt-1">Create Student</h6>
						</button>
					</form>
				</c:if>	
			</div>
			 ...
			 <table class="table table-striped table-dark">
			  ...
			  <c:forEach items="${students}" var="std">
			      ...
			      <td style="height: 150px;"><div
											class="scrollable custom-scrollbar">${std.getStdAdd()}</div></td>
				   ...
				   <td>
			      	 <form method="get" action="ServletStudent" class="pt-1">
			      		<button name="btnEdit" type="submit"
													value="${std.getStdId()}" class="btn btn-edit">
			      	 ...
			      	 <form method="post" action="ServletStudent" class="pt-2">
				      	<button name="postDelete" class="btn btn-delete"
													value="${std.getStdId()}" type="submit">
	...
	<c:if test="${createParam == 'selected'}">
		<div class="container create-block px-0 my-5">
			<div class="container-fluid px-0"> 
				...
				<c:if
						test="${registerStudentRole != null || sessionScope.role_id == 1 || sessionScope.role_id == 2}">
					<h1 class="xbootstrap">Register Student</h1>
				</c:if>
				
				<c:if
						test="${registerStudentRole == null && sessionScope.role_id == null}">
			...
			<div class="container py-5 w-50 form-container float-left">
				...
				<form method="post" action="ServletStudent"
						enctype="multipart/form-data">			  
				  <div class="form-row">
				    <div class="form-group col-md-4">
				      <label for="crStdFrst">First Name</label>
				      <input type="text" class="form-control" id="crStdFrst"
									name="crStdFrst" required>
				    </div>
				    <div class="form-group col-md-4">
				      <label for="crStdMid">Middle Name</label>
				      <input type="text" class="form-control" id="crStdMid"
									name="crStdMid">
				    </div>
				    <div class="form-group col-md-4">
				      <label for="crStdLast">Last Name</label>
				      <input type="text" class="form-control" id="crStdLast"
									name="crStdLast" required>
				    </div>
				 </div>
				 <div class="form-row">
				   <div class="form-group col-md-6">
				      <label for="crStdMail">Email</label>
				      <input type="email" class="form-control" id="crStdMail"
									name="crStdMail">
				    </div>
				    <div class="form-group col-md-6">
				      <label for="crStdPwd">Password</label>
					  <input type="password"
									class="form-control d-flex float-left stdPwdField"
									id="crStdPwd" name="crStdPwd" required>
					  <span class="pwdStatClass d-flex float-right mt-2"><i
									id="pwdIconStudentCr" class="fas fa-eye"></i></span>
				   </div>
			   </div>
			  ...
			  <div class="form-row">
			    <div class="form-group col-md-12">
			      ...
			      <textarea class="form-control" rows="5" id="crStdAdd"
									name="crStdAdd" required></textarea>
			    </div>
			  </div>
			  ...
			  <div class="form-row">
			    ...
			    <div class="col-md-6 form-group">
					...
					<select id="crStdGen" name="crStdGen" class="form-control"
			      ...
			      <input type="date" class="form-control" id="crStdDob"
									name="crStdDob" required>
			    </div>
			  ...
			  <div class="form-row">
			    ...
			    <div class="form-group col-md-4">
			      ...
			      <input type="text" class="form-control" id="crStdCont"
									name="crStdCont" required>
			    </div>
			    ...
			    <div class="form-group col-md-4">
			      ...
			      <input type="file" class="form-control d-none"
									onchange="showImageChoice(this)" id="crStdImg" name="crStdImg"
									required /> 
			    </div>
			    
			    <div class="form-group col-md-4">
				    <label for="crStdAttd">Attendance</label>
				    <select id="crStdAttd" name="crStdAttd" class="form-control"
			  ...
			  <div id="crStdImgView" class="form-row" style="display: none;">
			    ...
			    <div
		            ...
		            <div class="col-lg-4">
	                    <div class="member-card pt-2 pb-2">
	                        <div class="thumb-lg member-thumb mx-auto">
										<img src="" class="rounded-circle img-thumbnail show-image"
												alt="student-image-on-create-form">
							</div>
						</div>
					</div>
				</div>
					...
					<span
								class="text-danger bg-white error-message px-3 py-2 form-control mb-3"
								style="display: none;">
					</span>
			  </div>
			  ...
			  <div class="form-row">
			    <div class="form-group col-md-6">
			      ...
			      <select id="crStdDept" name="crStdDept" class="form-control"
									required>
			   ...
			   <div class="form-group col-md-6">
			      ...
			      <select id="crStdGrad" name="crStdGrad" class="form-control"
									required>
			      	<option value="Select">Select</option>
			  ...
			  <div class="form-row">
			    <div class="form-group col-md-6">
			      ...
			      <select id="crStdRole" name="crStdRole" class="form-control"
									required>
			      	<option value="Select">Select</option>
			    ...
			    <div class="form-group col-md-6">
			      ...
			      <select id="crStdSect" name="crStdSect" class="form-control"
									required>
			      	<option value="Select">Select</option>
			...
			<c:if test="${registerStudentRole != null}">
				<button name="postCreate" value="create" type="submit"
								class="btn btn-create" formaction="${studentRegisterServlet}">
					Create
				</button>
			</c:if>	 
		...
		<div class="container-fluid py-5 w-50 h-100 float-right mt-5">
			...
			<c:if
				...
				<form method="get" action="../commonPages/ServletHome">
					...
					<input id="homeRegParam" name="homepageRegisterParam"
								value="selected" class="d-none allView" type="submit" />
				</form>
			</c:if>
			...
			<c:if test="${sessionScope.role_id != null}">
				<form method="get" action="../commonPages/ServletAdmin">
					...
					<input id="btnView01" name="btnView01" value="selected"
								class="d-none allView" type="submit" />
				</form>
			</c:if>
		</div>
	</div>
	...
	<c:if test="${editParam == 'selected'}">
		<div class="container edit-block px-0 my-5">
			...
			<div class="container py-5 w-50 form-container float-left">
				...
				<div class="form-row">
				  ...
				  <div class="form-row">
				    ...
				    <div class="form-group col-md-6">
				      ...
				      <input type="email" class="form-control" id="upStdMail"
									name="upStdMail"
				    ...
				    <div class="form-group col-md-6">
					 ...
					 <input type="password"
									class="form-control d-flex float-left stdPwdField"
									id="upStdPwd" name="upStdPwd"
									value='<c:out value="${student.getStdPwd()}" />'>
					  <span class="pwdStatClass d-flex float-right mt-2"><i
									id="pwdIconStudentUp" class="fas fa-eye"></i></span>
				    </div>
				  </div>
				  ...
				  <div class="form-row">
				      ...
				      <textarea class="form-control" rows="5" id="upStdAdd"
									name="upStdAdd"><c:out
				  ...
				  <div class="form-row">
				    ...
				    <div class="col-md-6 form-group">
						...
						<select id="upStdGen" name="upStdGen" class="form-control"
									aria-label="Default select example">
						  <option value="Select"
										<c:if test='${student.getStdGen() == null}'><c:out value="selected" /></c:if>>
						  	Select
					  	  </option>
						  <option value="Male"
										<c:if test='${student.getStdGen() == "Male"}'><c:out value="selected" /></c:if>>
							Male
						 </option>
						 <option value="Female"
										<c:if test='${student.getStdGen() == "Female"}'><c:out value="selected" /></c:if>>
							Female
						 </option>
						 <option value="Transexual"
										<c:if test='${student.getStdGen() == "Transexual"}'><c:out value="selected" /></c:if>>
							Transexual
						 </option>
						 <option value="Bisexual"
										<c:if test='${student.getStdGen() == "Bisexual"}'><c:out value="selected" /></c:if>>
							Bisexual
						 </option>
						 <option value="Gay"
										<c:if test='${student.getStdGen() == "Gay"}'><c:out value="selected" /></c:if>>
							Gay
						 </option>
						 <option value="Lesbian"
										<c:if test='${student.getStdGen() == "Lesbian"}'><c:out value="selected" /></c:if>>
							LESBIAN
						 </option>
				      ...
				      <input type="date" class="form-control" id="upStdDob"
									name="upStdDob"
									value='<c:out value="${student.getStdDob()}" />'>
				    </div>
				  </div>
				  ...
				  <div class="form-row">
				    <div class="form-group col-md-4">
				      ...
				      <input type="text" class="form-control" id="upStdCont"
									name="upStdCont"
									value='<c:out value="${student.getStdCont()}" />'>
				    </div>
				  ...
				  <div class="form-group col-md-4">
				      ...
				      <input type="file" class="form-control d-none"
									onchange="showImageChoice(this)" id="upStdImg" name="upStdImg" /> 
			       </div>
				    
				   <div class="form-group col-md-4">
					    <label for="upStdAttd">Attendance</label>
					    <select id="upStdAttd" name="upStdAttd" class="form-control"
									aria-label="Default select example">
							<option value="Select"
										<c:if test='${student.getStdAttd() == null}'><c:out value="selected" /></c:if>>
							  	Select
						  	</option>
							<option value="Less than 20%"
										<c:if test='${student.getStdAttd() == "Less than 20%"}'><c:out value="selected" /></c:if>>
								Less than 20%
							</option>
							<option value="Less than 50%"
										<c:if test='${student.getStdAttd() == "Less than 50%"}'><c:out value="selected" /></c:if>>
								Less than 50%
							</option>
							<option value="(50 - 70)%"
										<c:if test='${student.getStdAttd() == "(50 - 70)%"}'><c:out value="selected" /></c:if>>
								(50 - 70)%
							</option>
							<option value="(70 - 80)%"
										<c:if test='${student.getStdAttd() == "(70 - 80)%"}'><c:out value="selected" /></c:if>>
								(70 - 80)%
							</option>
							<option value="(80 - 90)%"
										<c:if test='${student.getStdAttd() == "(80 - 90)%"}'><c:out value="selected" /></c:if>>
								(80 - 90)%
							</option>
							<option value="More than 90%"
										<c:if test='${student.getStdAttd() == "More than 90%"}'><c:out value="selected" /></c:if>>
								More than 90%
							</option>
						</select>
					  </div>
				    ...
				    <div
			            ...
			            <div class="col-lg-4">
		                    <div class="member-card pt-2 pb-2">
		                        <div class="thumb-lg member-thumb mx-auto">
											<img
												src="<c:url value='/resources/images/${student.getStdImg()}' />"
												class="rounded-circle img-thumbnail show-image"
												alt="student-image-on-edit-form">
								</div>
							</div>
						</div>
					</div>
				  ...
				  <div class="form-row">
				    <div class="form-group col-md-6">
				      ...
				      <select id="upStdDept" name="upStdDept" class="form-control">
				      	<option value="Select">Select</option>
				      	<c:forEach items="${depNames}" var="depUp">
				      		<option value='<c:out value="${depUp}" />'
											<c:if test="${serviceStudent.getDepNameByDepId(student.getStdDept()) == depUp}">
				    ...
				    <div class="form-group col-md-6">
				      ...
				      <select id="upStdGrad" name="upStdGrad" class="form-control">
				      	<option value="Select">Select</option>
			      		<c:forEach items="${gradeScores}" var="gradeUp">					
							<option value='<c:out value="${gradeUp}" />'
											<c:if test="${serviceStudent.getGradeByGradeId(student.getStdGrad()) == gradeUp}">
												<c:out value="selected" />
											</c:if>>
				      			<c:out value="${gradeUp}" />
							</option>
				  ...
				  <div class="form-row">
				    <div class="form-group col-md-6">
				      ...
				      <select id="upStdRole" name="upStdRole" class="form-control">
				      	<option value="Select">Select</option>
			      		<c:forEach items="${roleNames}" var="roleUp">					
							<option value='<c:out value="${roleUp}" />'
											<c:if test="${serviceStudent.getRoleNameByRoleId(student.getStdRole()) == roleUp}">
												<c:out value="selected" />
											</c:if>
				    ...
				    <div class="form-group col-md-6">
				      ...
				      <select id="upStdSect" name="upStdSect" class="form-control">
				      	<option value="Select">Select</option>
			      		<c:forEach items="${sectionRooms}" var="secUp">					
							<option value='<c:out value="${secUp}" />'
											<c:if test="${serviceStudent.getSectionBySecId(student.getStdSect()) == secUp}">
												<c:out value="selected" />
											</c:if>>
								<c:out value="${secUp}" />
							</option>
				...
				<c:if test="${registerStudentRole == null}">
				  <button name="postUpdate" value="${student.getStdId()}"
								type="submit" class="btn btn-edit">
						Update
				</button>
				</c:if>	
			...
			<div class="container-fluid py-5 w-50 h-100 float-right mt-5">
				...
				<c:if test="${sessionScope.role_id != null}">
					<form method="get" action="../commonPages/ServletAdmin">
						...
						<input id="btnView02" name="btnView02" value="selected"
								class="d-none allView" type="submit" />
					</form>
				</c:if>
			</div>
			...
			<div
				...
				<c:if test="${sessionScope.role_id != null}">
					<form method="get" action="../commonPages/ServletLogin">
						<button name="btnView01" type="submit" value="create"
								class="btn btn-success ml-5">
				...
				<table class="table table-striped table-dark w-50 text-left mt-5">
				  <thead>
				    <tr class="text-center title-view bg-primary">
					  <th scope="row" colspan="2" class="pt-3"><h2>Student View</h2></th>
				    </tr>
				  </thead>
				  <tbody>
				  	<tr>
		            	<td colspan="2" class="text-center">
			            	<img
									src="<c:url value='/resources/images/${student.getStdImg()}' />"
									class="rounded-circle img-thumbnail show-image"
									alt="student-image-on-view-details" width="100" height="100">
						</td>
				  	</tr>
				  	<tr>
				      <td scope="col">Id</td>
				    ...
				    <c:if
								test="${sessionScope.role_id == 1 || sessionScope.student_id == student.getStdId()}">
					    <tr>
				    ...
				    <tr>
				      ...
				      <td style="height: 150px;"><div
										class="scrollable custom-scrollbar">${student.getStdAdd()}</div></td>
				    </tr>
				    <tr>
