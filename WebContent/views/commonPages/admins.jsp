...
<t:layout>
	<jsp:attribute name="title">
		...
		<c:if
			test="${createParam == 'selected'  && registerAdminRole == null}">
			  ...
			  <c:forEach items="${admins}" var="adm">
			      ...
			      <td>${adm.getAdmFrst()} ${adm.getAdmMid()} ${adm.getAdmLast()}</td>
			      			      <td style="height: 150px;"><div
											class="scrollable custom-scrollbar">${adm.getAdmAdd()}</div></td>
			      <td>${adm.getAdmGen()}</td>
				      ...
				      <td>
				      	<form method="get" action="ServletAdmin" class="pt-3">
				      		<button name="btnEdit" type="submit"
													value="${adm.getAdmId()}" class="btn btn-edit">
				      	...
				      	<form method="post" action="ServletAdmin" class="pt-2">
				      		<button name="postDelete" class="btn btn-delete"
													value="${adm.getAdmId()}" type="submit">
	...
	<c:if test="${createParam == 'selected'}">
		...
		<div class="container create-block px-0 my-5">
				...
				<form method="post" action="ServletAdmin"
					...
					<div class="form-row">
						<div class="form-group col-md-4">
							<label for="rgAdmFrst">First Name</label> <input type="text"
									class="form-control" id="rgAdmFrst" name="rgAdmFrst">
						</div>
						<div class="form-group col-md-4">
							<label for="rgAdmMid">Middle Name</label> <input type="text"
									class="form-control" id="rgAdmMid" name="rgAdmMid">
						</div>
						<div class="form-group col-md-4">
							<label for="rgAdmLast">Last Name</label> <input type="text"
									class="form-control" id="rgAdmLast" name="rgAdmLast">
						</div>
					</div>
					<div class="form-row">
						<div class="form-group col-md-6">
							<label for="rgAdmMail">Email</label> <input type="email"
									class="form-control" id="rgAdmMail" name="rgAdmMail">
						</div>
						<div class="form-group col-md-6">
							<label for="rgAdmPwd">Password</label> <input type="password"
									class="form-control d-flex float-left admPwdField"
									id="rgAdmPwd" name="rgAdmPwd"> <span
									class="pwdStatClass d-flex float-right mt-2"><i
					...
					<div class="form-row">
						<div class="form-group col-md-12">
							...
							<textarea class="form-control" rows="5" id="rgAdmAdd"
									name="rgAdmAdd"></textarea>
						</div>
					</div>

					<div class="form-row">
						<div class="col-md-6 form-group">
							<label for="rgAdmGen">Gender</label> <select id="rgAdmGen"
									name="rgAdmGen" class="form-control"
						...
						<div class="form-group col-md-6">
							<label for="rgAdmDob">Date of Birth</label> <input type="date"
									class="form-control" id="rgAdmDob" name="rgAdmDob">
						</div>
					</div>

					<div class="form-row">
						<div class="form-group col-md-4">
							<label for="rgAdmCont">Contact</label> <input type="text"
									class="form-control" id="rgAdmCont" name="rgAdmCont">
						</div>
						<div class="form-group col-md-4">
							...
							</label> <input type="file" class="form-control d-none"
									onchange="showImageChoice(this)" id="rgAdmImg" name="rgAdmImg" />
						</div>

						<div class="form-group col-md-4">
							<label for="rgAdmRole">Role</label> <select id="rgAdmRole"
									name="rgAdmRole" class="form-control">
								<option value="Select">Select</option>
								<c:forEach items="${roleNames}" var="roleCr">
									<option value='<c:out value="${roleCr}" />'
								...
								<div class="member-card pt-2 pb-2">
									<div class="thumb-lg member-thumb mx-auto">
										<img src="" class="rounded-circle img-thumbnail show-image"
												alt="admin-image-on-register-form">
									</div>
								</div>
							</div>
						</div>
					...
					<c:if test="${registerAdminRole != null}">
						<button name="postCreate" value="create" type="submit"
								class="btn btn-create" formaction="${adminRegisterServlet}">
							Create
						</button>
					</c:if>	 
			...
			<div class="container-fluid py-5 w-50 h-100 float-right mt-5">
				<c:if
						test="${registerAdminRole != null && sessionScope.role_id == null}">
				...
				<c:if
					...
					<form method="get" action="ServletAdmin">
						...
						<input id="viewParam" name="viewParam" value="view"
	...
	<c:if test="${editParam == 'selected'}">
			...
			<div class="container py-5 w-50 form-container float-left">
				...
				<div class="form-row">
				    ...
				    <div class="form-group col-md-4">
				      ...
				      <input type="text" class="form-control" id="rgAdmFrst"
									name="rgAdmFrst"
									value='<c:out value="${admin.getAdmFrst()}" />'>
				    </div>
				    
				    <div class="form-group col-md-4">
				      <label for="rgAdmMid">Middle Name</label>
				      <input type="text" class="form-control" id="rgAdmMid"
									name="rgAdmMid" value='<c:out value="${admin.getAdmMid()}" />'>
				    </div>
				    
				    <div class="form-group col-md-4">
				      <label for="rgAdmLast">Last Name</label>
				      <input type="text" class="form-control" id="rgAdmLast"
									name="rgAdmLast"
									value='<c:out value="${admin.getAdmLast()}" />'>
				    </div>
				  </div>
				  ...
				  <div class="form-row">
				    <div class="form-group col-md-6">
				      ...
				      <input type="email" class="form-control" id="rgAdmMail"
									name="rgAdmMail"
									value='<c:out value="${admin.getAdmMail()}" />'>
				    </div>
				    ...
				    <div class="form-group col-md-6">
				      ...
				      <input type="password"
									class="form-control d-flex float-left admPwdField"
									id="rgAdmPwd" name="rgAdmPwd"
									value='<c:out value="${admin.getAdmPwd()}" />'>
					  <span class="pwdStatClass d-flex float-right mt-2"><i
									id="pwdIconAdminUp" class="fas fa-eye"></i></span>
				  ...
				  <div class="form-row">
				    ...
				    <div class="form-group col-md-12">
				      <label for="rgAdmAdd">Address</label>
				      <textarea class="form-control" rows="5" id="rgAdmAdd"
									name="rgAdmAdd"><c:out value="${admin.getAdmAdd()}" /></textarea>
				    </div>
				  ...
				  <div class="form-row">
				  	...
				  	<div class="col-md-6 form-group">
						...
						<select id="rgAdmGen" name="rgAdmGen" class="form-control"
									aria-label="Default select example">
						  <option value="Select"
										<c:if test='${admin.getAdmGen() == null}'><c:out value="selected" /></c:if>>
						  	Select
					  	  </option>
						  <option value="Male"
										<c:if test='${admin.getAdmGen() == "Male"}'><c:out value="selected" /></c:if>>
							Male
						 </option>
						 <option value="Female"
										<c:if test='${admin.getAdmGen() == "Female"}'><c:out value="selected" /></c:if>>
							Female
						 </option>
						  
						<option value="Transexual"
										<c:if test='${admin.getAdmGen() == "Transexual"}'><c:out value="selected" /></c:if>>
							Transexual
						 </option>
						 <option value="Bisexual"
										<c:if test='${admin.getAdmGen() == "Bisexual"}'><c:out value="selected" /></c:if>>
							Bisexual
						 </option>
						 <option value="Gay"
										<c:if test='${admin.getAdmGen() == "Gay"}'><c:out value="selected" /></c:if>>
							Gay
						 </option>
						 <option value="Lesbian"
										<c:if test='${admin.getAdmGen() == "Lesbian"}'><c:out value="selected" /></c:if>>
							Lesbian
						 </option>
						</select>
				  	</div>
				      ...
				      <input type="date" class="form-control" id="rgAdmDob"
									name="rgAdmDob" value='<c:out value="${admin.getAdmDob()}" />'>
				   	</div>
				  </div>
				  ...
				  <div class="form-row">
				    ...
				    <div class="form-group col-md-4">
				      ...
				      <input type="text" class="form-control" id="rgAdmCont"
									name="rgAdmCont"
									value='<c:out value="${admin.getAdmCont()}" />'>
				    </div>
				    ...
				    <div class="form-group col-md-4">
				      ...
				      <select id="rgAdmRole" name="rgAdmRole" class="form-control">
				      	<option value="Select">Select</option>
			      		<c:forEach items="${roleNames}" var="roleUp">
				      		<option value='<c:out value="${roleUp}" />'
											<c:if test="${serviceAdmin.getRoleNameByRoleId(admin.getAdmRole()) == roleUp}">
				    ...
				    <div
			            ...
			            <div class="col-lg-4">
		                    <div class="member-card pt-2 pb-2">
		                        <div class="thumb-lg member-thumb mx-auto">
											<img
												src="<c:url value='/resources/images/${admin.getAdmImg()}' />"
												class="rounded-circle img-thumbnail show-image"
												alt="admin-image-on-edit-form">
								</div>
							</div>
						</div>
					</div>
				...
				<c:if test="${registerAdminRole == null}">
				  <button name="postUpdate" value="${admin.getAdmId()}"
				...
				<c:if test="${registerAdminRole != null}">
					<button name="postUpdate" value="${admin.getAdmId()}" type="submit"
								class="btn btn-edit" formaction="${adminEditProfileServlet}">
						Update
					</button>
				</c:if>	
		...
		<div class="container-fluid py-5 w-50 h-100 float-right mt-5">
			<form method="get" action="ServletAdmin">
				...
				<input id="viewParam" name="viewParam" class="d-none allView"
						type="submit" />
			</form>
		</div>
	</c:if>
				...
				<table class="table table-striped table-dark w-50 text-left mt-5">
				  <thead>
				    <tr class="text-center title-view bg-primary">
					  <th scope="row" colspan="2" class="pt-3"><h2>Admin View</h2></th>
				    </tr>
				  </thead>
				  <tbody>
				  	<tr>
				    ...
				    <c:if
								test="${sessionScope.admin_id == 1 || sessionScope.admin_id == admin.getAdmId()}">
					    <tr>
				    ...
				    <tr>
				      ...
				      <td style="height: 150px;"><div
										class="scrollable custom-scrollbar">${admin.getAdmAdd()}</div></td>
				    </tr>
				    <tr>
